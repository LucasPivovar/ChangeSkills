...
file_path = r"c:\Users\patov\Documents\Projects\ChangeSkills\ChangeSkills\services\aulas-conversacao.html"
...
new_form = '''
                    ...
                    <form id="wizard-form" style="display: flex; flex-direction: column; gap: 1.2rem;" onsubmit="event.preventDefault(); alert('Solicitação enviada com sucesso!');">
                        ...
                        <!-- ETAPA 4: Foco e Nível -->
                        <div class="form-step" id="step-4" style="display: none;">
                            <h3 style="color: var(--primary-dark); font-size: 1.5rem; font-weight: 700; margin-bottom: 1.5rem; text-align: left;">4. Sobre Você</h3>
                            
                            <div style="margin-bottom: 1.2rem;">
                                <label style="display: block; font-weight: 600; color: var(--primary-dark); margin-bottom: 0.5rem; font-size: 0.95rem;">Qual seu nível atual de inglês?</label>
                                <select required style="width: 100%; padding: 1.1rem 1.5rem; border-radius: 12px; border: 1px solid rgba(0,0,0,0.08); background: #f8fafc; font-size: 1rem; outline: none; transition: border-color 0.3s;">
                                    ...
                                    <option>Nunca fiz curso</option>
                                    <option>Sei um pouco (< 1 ano de curso)</option>
                                    <option>Sei moderadamente (> 1 ano de curso)</option>
                                    <option>Sou fluente mas preciso praticar!</option>
                                </select>
                            </div>
                            
                            <div style="margin-bottom: 1.2rem;">
                                <label style="display: block; font-weight: 600; color: var(--primary-dark); margin-bottom: 0.5rem; font-size: 0.95rem;">Qual é o foco da sua conversação?</label>
                                <textarea rows="4" placeholder="Ex: Preciso de inglês para o trabalho, viagens, entrevistas..." style="width: 100%; padding: 1.1rem 1.5rem; border-radius: 12px; border: 1px solid rgba(0,0,0,0.08); background: #f8fafc; font-size: 1rem; outline: none; transition: border-color 0.3s; resize: vertical;"></textarea>
                            </div>
...
# Replace the whole <form id="conversacao-form" ...> ... </form> section
pattern = re.compile(r'<form id="conversacao-form".*?</form>', re.DOTALL)
new_content = pattern.sub(new_form, content)

# Remove any existing nextStep, prevStep functions if present
old_script_pattern = re.compile(r'// Wizard Logic.*?function prevStep.*?}', re.DOTALL)
new_content = old_script_pattern.sub('', new_content)

# Remove the inline styles that we don't need for wizard anymore since we use the updated ones
style_pattern = re.compile(r'/\* Wizard Styles \*/.*?.btn-next:hover \{ background: var\(--primary-dark\); \}', re.DOTALL)
...
print("Updated aulas-conversacao.html")
