...
files.forEach(file => {
  ...
  // Calculate coursePath dynamically
  let coursePath = '/cursos/ingles';
  if (file.startsWith('espanhol-')) coursePath = '/cursos/espanhol';
  else if (file.startsWith('frances-')) coursePath = '/cursos/frances';
  else if (file.startsWith('portugues-')) coursePath = '/cursos/portugues';
  ...
  mainHtml = mainHtml.replace(/<a\s+([^>]*to="[^"]+"[^>]*)>([\s\S]*?)<\/a>/g, '<RouterLink $1>$2</RouterLink>');
...
<template>
  <div class="modality-detail-view">
    <div class="back-nav">
      <div class="container">
        <RouterLink to="${coursePath}" class="back-link">
